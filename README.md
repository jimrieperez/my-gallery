...
VITE_Public_Key=your-supabase-public-api-key
VITE_STORAGE_BUCKET=images
```

5. Configure Storage in the Supabase SQL Editor by running [`supabase-storage-policies.sql`](supabase-storage-policies.sql). This creates the `images` bucket and allows authenticated users to upload, view, and delete files only in their own folder.
